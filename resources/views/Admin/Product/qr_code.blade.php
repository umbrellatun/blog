            ...
            {{-- @for ($i=0; $i < $QRCode->in_stock; $i++) --}}
                <div class="" style="float: left; width: 80%; height: 100%; padding-top: 1.5cm; margin-left: 10%;">
                    ...
                    <table border="1" style="width: 92%;" cellpadding="0" cellspacing="0">
                        <tr>
                            <td colspan="2">&nbsp;Company: <b>{{$QRCode->Company->name}}</b></td>
                        </tr>
                        <tr>
                            <td colspan="2">&nbsp;SKU: <b>{{$QRCode->sku}}</b></td>
                        </tr>
                        <tr>
                            <td colspan="2">&nbsp;Name : <b>{{$QRCode->name}}</b></td>
                        </tr>
                        <tr>
                            <td colspan="2">&nbsp;Type : <b>{{$QRCode->ProductType->name}}</b></td>
                        </tr>
                        <tr>
                            <!-- set date  -->
                            <td colspan="2">&nbsp;Create Date: <b>{{ date_format( $QRCode->created_at, 'Y-m-d')}}</b></td>
                        </tr>
                    </table>
                </div>
            {{-- @endfor --}}
